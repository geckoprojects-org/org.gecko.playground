 ...
 */
package org.geckoprojects.log4j.impl;
 ...
 */
@Capability(namespace = LogManagerProvider.CAPABILITY_NAMESPACE, name = LogManagerProvider.LOGGING_LOG4J, version = "2.0")
@Requirements({
	@Requirement(namespace = "osgi.identity", name = "slf4j.api", filter = "(&(version>=1.7.30)(!(version>=2.0)))"),
	@Requirement(namespace = "osgi.identity", name = "org.apache.logging.log4j.api", filter = "(&(version>=2.17)(!(version>=3.0)))"),
	@Requirement(namespace = "osgi.identity", name = "org.apache.logging.log4j.core", filter = "(&(version>=2.17)(!(version>=3.0)))"),
	@Requirement(namespace = "osgi.identity", name = "org.apache.logging.log4j.slf4j-impl", filter = "(&(version>=2.17)(!(version>=3.0)))"),
	@Requirement(namespace = "osgi.identity", name = "org.apache.logging.log4j.jul", filter = "(&(version>=2.17)(!(version>=3.0)))"),
})
@Component
public class LogManager implements LogManagerProvider {

    /**
     * The name of the root Logger.
     */
    public static final String ROOT_LOGGER_NAME = Strings.EMPTY;
    private static final LoggerContextFactory SIMPLE_FACTORY = new SimpleLoggerContextFactory();
    
    private final Logger LOGGER = StatusLogger.getLogger();
    private final String FQCN = LogManager.class.getName();
    private final AtomicReference<LoggerContextFactory> factoryRef = new AtomicReference<LoggerContextFactory>(SIMPLE_FACTORY);
    
    @Activate
    public void activate(BundleContext context) {
    }

    @Reference(cardinality = ReferenceCardinality.OPTIONAL, policy = ReferencePolicy.DYNAMIC, unbind = "removeProvider")
    public void addProvider(Provider provider) {
    	final Class<? extends LoggerContextFactory> factoryClass = provider.loadLoggerContextFactory();
        if (factoryClass != null) {
            try {
            	LoggerContextFactory factory = factoryClass.getDeclaredConstructor(new Class<?>[0]).newInstance();
            	LoggerContextFactory current = factoryRef.get();
            	if (!factory.equals(current)) {
            		factoryRef.compareAndSet(current, factory);
            	}
    ...
    public void removeProvider(Provider provider) {
    	LoggerContextFactory contextFactory = factoryRef.get();
    	factoryRef.compareAndSet(contextFactory, SIMPLE_FACTORY);
    }
    ...
    public LoggerContext getContext() {
        try {
            return factoryRef.get().getContext(FQCN, null, null, true);
    ...
    public  LoggerContext getContext(final boolean currentContext) {
        ...
        try {
            return factoryRef.get().getContext(FQCN, null, null, currentContext, null, null);
        } catch (final IllegalStateException ex) {
            LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
    ...
    public  LoggerContext getContext(final ClassLoader loader, final boolean currentContext) {
        try {
            return factoryRef.get().getContext(FQCN, loader, null, currentContext);
        } catch (final IllegalStateException ex) {
            LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
    ...
    public  LoggerContext getContext(final ClassLoader loader, final boolean currentContext,
        ...
        try {
            return factoryRef.get().getContext(FQCN, loader, externalContext, currentContext);
        } catch (final IllegalStateException ex) {
            LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
    ...
    public  LoggerContext getContext(final ClassLoader loader, final boolean currentContext,
        ...
        try {
            return factoryRef.get().getContext(FQCN, loader, null, currentContext, configLocation, null);
        } catch (final IllegalStateException ex) {
            LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
    ...
    public  LoggerContext getContext(final ClassLoader loader, final boolean currentContext,
        ...
        try {
            return factoryRef.get().getContext(FQCN, loader, externalContext, currentContext, configLocation, null);
        } catch (final IllegalStateException ex) {
            LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
            return new SimpleLoggerContextFactory().getContext(FQCN, loader, externalContext, currentContext,
                    configLocation, null);
    ...
    public  LoggerContext getContext(final ClassLoader loader, final boolean currentContext,
        ...
        try {
            return factoryRef.get().getContext(FQCN, loader, externalContext, currentContext, configLocation, name);
        } catch (final IllegalStateException ex) {
            LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
            return new SimpleLoggerContextFactory().getContext(FQCN, loader, externalContext, currentContext,
                    configLocation, name);
     ...
     * @return a LoggerContext.
     */
    protected  LoggerContext getContext(final String fqcn, final boolean currentContext) {
        try {
            return factoryRef.get().getContext(fqcn, null, null, currentContext);
        } catch (final IllegalStateException ex) {
            LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
     ...
     * @return a LoggerContext.
     */
    protected  LoggerContext getContext(final String fqcn, final ClassLoader loader,
    ...
    protected  LoggerContext getContext(final String fqcn, final ClassLoader loader,
        ...
        try {
            return factoryRef.get().getContext(fqcn, loader, null, currentContext, configLocation, name);
        } catch (final IllegalStateException ex) {
            LOGGER.warn(ex.getMessage() + " Using SimpleLogger");
            return new SimpleLoggerContextFactory().getContext(fqcn, loader, null, currentContext);
        }
    ...
    public  void shutdown(final boolean currentContext) {
        factoryRef.get().shutdown(FQCN, null, currentContext, false);
    }
    ...
    public  void shutdown(final boolean currentContext, final boolean allContexts) {
    	factoryRef.get().shutdown(FQCN, null, currentContext, allContexts);
    }
    ...
    public  void setFactory(final LoggerContextFactory factory) {
        this.factoryRef.set(factory);
    }
    ...
    private  Class<?> callerClass(final Class<?> clazz) {
        if (clazz != null) {
            return clazz;
        }
     ...
     * @throws UnsupportedOperationException if the calling class cannot be determined.
     */
    public  Logger getLogger(final MessageFactory messageFactory) {
     ...
     * @return The Logger.
     */
    protected  Logger getLogger(final String fqcn, final String name) {
        return factoryRef.get().getContext(fqcn, null, null, false).getLogger(name);
    }
